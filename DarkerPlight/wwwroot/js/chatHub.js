var app = new Vue({
    ...
    methods: {
        enterGroup: function () {
            ...
            //set message
            self.chatHeaderMsg = "Hello stake holder - " + self.username;
            self.staticUsername = "";

            //get messages of the selected user
            axios.get('/api/apphub/getgroupmessage?groupNumber=' + 1)
            ...
            axios({
                ...
                .then(function (response) {
                    if (self.groupNumber > 0) {
                        connection.invoke("SendMessageToGroup", "PrivateGroup", message, self.username, "group").catch(function (err) {
                            return console.error(err.toString());
                        });
                    }
                    else {
                        if (self.staticUsername != null) {
        ...
        getChatHistory: function (userIdone,userIdTwo) {
            var self = this;
            const myNode = document.getElementById("chatMessages");
            while (myNode.firstChild) {
                myNode.removeChild(myNode.lastChild);
            ...
            axios.get('/api/apphub/getmessage?userIdOne=' + userIdone + '&userIdTwo=' + userIdTwo)
                .then(function (response) {
                    if (response.data) {
                        var messages = response.data;
                        self.messageCount = + 1;
...
connection.on("RecieveMessage", function (message, senderId, username) {
    ...
    app.$data.chatHeaderMsg = "Chat with user"
    app.$data.staticUsername = username;
    app.$data.groupNumber = 0;
    ...
    if (app.$data.contactList.filter(e => e.username === username).length === 0) {
        ...
        //get history chat
        app.getChatHistory(app.$data.username, username);
    }
    else {
        //existing contact chat
        var time = getCurrentDateTime();
        //push to top list with online status
